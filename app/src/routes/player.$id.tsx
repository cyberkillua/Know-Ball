...
import type { Player, MatchRating, PeerRating, PlayerStats, PlayerUnderstat, Shot } from '../lib/types'
...
function PlayerProfilePage() {
  ...
  const passesCompletedPer90 = stats && stats.minutes > 0 ? (passesCompleted / stats.minutes) * 90 : 0
  const passingAccuracy = passesTotal > 0 ? passesCompleted / passesTotal : null
  const accurateLongBallsPer90 = stats && stats.minutes > 0 ? ((stats.accurate_long_balls ?? 0) / stats.minutes) * 90 : 0
  const longBallAccuracy = (stats?.total_long_balls ?? 0) > 0 ? (stats?.accurate_long_balls ?? 0) / (stats?.total_long_balls ?? 1) : null
              ...
              <CardContent className="p-6">
                ...
                <div style={{ marginBottom: 24 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                    ...
                    <div ref={controlsRef} style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {/* Stat mode toggle */}
                ...
                {!peerQualified && stats ? (
                  ...
                  <div style={{ marginBottom: 24 }}>
                    <PizzaChart
                      data={peerQualified ? (isST ? [
                        ...
                        { label: statMode === 'per90' ? 'BCM/90' : 'BCM', percentile: pct(peerRating?.big_chances_missed_percentile, peerRating?.big_chances_missed_raw_percentile, statMode, peerQualified), inverted: true },
                        // Chance creation (4)
                        ...
                        { label: statMode === 'per90' ? 'Pass/90' : 'Passes', percentile: pct(peerRating?.passes_completed_per90_percentile, peerRating?.passes_completed_raw_percentile, statMode, peerQualified) },
                        { label: 'Pass%', percentile: peerRating?.passing_accuracy_percentile ?? 0 },
                      ] : isCAM ? [
                        // Chance creation (6)
                        { label: statMode === 'per90' ? 'xA/90' : 'xA', percentile: pct(peerRating?.xa_per90_percentile, peerRating?.xa_raw_percentile, statMode, peerQualified) },
                        { label: statMode === 'per90' ? 'Ast/90' : 'Ast', percentile: pct(peerRating?.assists_per90_percentile, peerRating?.assists_raw_percentile, statMode, peerQualified) },
                        ...
                        { label: statMode === 'per90' ? 'Pass/90' : 'Passes', percentile: pct(peerRating?.passes_completed_per90_percentile, peerRating?.passes_completed_raw_percentile, statMode, peerQualified) },
                        { label: 'Pass%', percentile: peerRating?.passing_accuracy_percentile ?? 0 },
                        { label: statMode === 'per90' ? 'LB/90' : 'LB', percentile: pct(peerRating?.accurate_long_balls_per90_percentile, peerRating?.accurate_long_balls_raw_percentile, statMode, peerQualified) },
                      ] : isWinger ? [
                        // Chance creation (5)
                        { label: statMode === 'per90' ? 'xA/90' : 'xA', percentile: pct(peerRating?.xa_per90_percentile, peerRating?.xa_raw_percentile, statMode, peerQualified) },
                        { label: statMode === 'per90' ? 'Ast/90' : 'Ast', percentile: pct(peerRating?.assists_per90_percentile, peerRating?.assists_raw_percentile, statMode, peerQualified) },
                        ...
                        { label: statMode === 'per90' ? 'Pass/90' : 'Passes', percentile: pct(peerRating?.passes_completed_per90_percentile, peerRating?.passes_completed_raw_percentile, statMode, peerQualified) },
                        { label: 'Pass%', percentile: peerRating?.passing_accuracy_percentile ?? 0 },
                        ...(peerRating?.xg_chain_per90_percentile != null ? [{ label: statMode === 'per90' ? 'xChain/90' : 'xChain', percentile: pct(peerRating?.xg_chain_per90_percentile, peerRating?.xg_chain_raw_percentile, statMode, peerQualified) }] : [{ label: statMode === 'per90' ? 'LB/90' : 'LB', percentile: pct(peerRating?.accurate_long_balls_per90_percentile, peerRating?.accurate_long_balls_raw_percentile, statMode, peerQualified) }]),
                      ] : isDefensiveWinger ? [
                        // Chance creation (4)
                        { label: statMode === 'per90' ? 'xA/90' : 'xA', percentile: pct(peerRating?.xa_per90_percentile, peerRating?.xa_raw_percentile, statMode, peerQualified) },
                        { label: statMode === 'per90' ? 'Ast/90' : 'Ast', percentile: pct(peerRating?.assists_per90_percentile, peerRating?.assists_raw_percentile, statMode, peerQualified) },
                        ...
                        { label: 'Pass%', percentile: peerRating?.passing_accuracy_percentile ?? 0 },
                        { label: statMode === 'per90' ? 'LB/90' : 'LB', percentile: pct(peerRating?.accurate_long_balls_per90_percentile, peerRating?.accurate_long_balls_raw_percentile, statMode, peerQualified) },
                        { label: 'LB%', percentile: peerRating?.long_ball_accuracy_percentile ?? 0 },
                        ...(peerRating?.xg_chain_per90_percentile != null ? [{ label: statMode === 'per90' ? 'xChain/90' : 'xChain', percentile: pct(peerRating?.xg_chain_per90_percentile, peerRating?.xg_chain_raw_percentile, statMode, peerQualified) }] : []),
                        ...(peerRating?.xg_buildup_per90_percentile != null ? [{ label: statMode === 'per90' ? 'xBuildup/90' : 'xBuildup', percentile: pct(peerRating?.xg_buildup_per90_percentile, peerRating?.xg_buildup_raw_percentile, statMode, peerQualified) }] : []),
                      ] : isCM ? [
                        // Chance creation (5)
                        { label: statMode === 'per90' ? 'xA/90' : 'xA', percentile: pct(peerRating?.xa_per90_percentile, peerRating?.xa_raw_percentile, statMode, peerQualified) },
                        { label: statMode === 'per90' ? 'Ast/90' : 'Ast', percentile: pct(peerRating?.assists_per90_percentile, peerRating?.assists_raw_percentile, statMode, peerQualified) },
                  ...
                  <>
                    ...
                    <div style={{ marginBottom: 32 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Goalscoring</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2px' }}>
                        ...
                        <StatRow label={statMode === 'per90' ? 'xGOT per 90' : 'xGOT'} value={fmt(stats.xgot_per90)} percentile={pct(peerRating?.xgot_per90_percentile, peerRating?.xg_raw_percentile, statMode, peerQualified)} />
                    ...
                    <div style={{ marginBottom: 32 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Chance creation</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2px' }}>
                    ...
                    <div style={{ marginBottom: 32 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Passing</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2px' }}>
                        <StatRow label={statMode === 'per90' ? 'Passes /90' : 'Passes'} value={statMode === 'per90' ? fmt(passesCompletedPer90, 1) : String(passesCompleted)} percentile={pct(peerRating?.passes_completed_per90_percentile, peerRating?.passes_completed_raw_percentile, statMode, peerQualified)} />
                        <StatRow label="Passing accuracy" value={fmtPct(passingAccuracy)} percentile={peerQualified ? (peerRating?.passing_accuracy_percentile ?? 0) : 0} />
                        {understat && <StatRow label={statMode === 'per90' ? 'xG Chain /90' : 'xG Chain'} value={statMode === 'per90' ? fmt(understat.xg_chain_per90) : fmt(understat.xg_chain)} percentile={pct(peerRating?.xg_chain_per90_percentile, peerRating?.xg_chain_raw_percentile, statMode, peerQualified)} />}
                        {understat && <StatRow label={statMode === 'per90' ? 'xG Buildup /90' : 'xG Buildup'} value={statMode === 'per90' ? fmt(understat.xg_buildup_per90) : fmt(understat.xg_buildup)} percentile={pct(peerRating?.xg_buildup_per90_percentile, peerRating?.xg_buildup_raw_percentile, statMode, peerQualified)} />}
                        <StatRow label={statMode === 'per90' ? 'Accurate long balls /90' : 'Accurate long balls'} value={statMode === 'per90' ? fmt(accurateLongBallsPer90, 1) : String(stats.accurate_long_balls ?? 0)} percentile={pct(peerRating?.accurate_long_balls_per90_percentile, peerRating?.accurate_long_balls_raw_percentile, statMode, peerQualified)} />
                        <StatRow label="Long ball accuracy" value={fmtPct(longBallAccuracy)} percentile={peerQualified ? (peerRating?.long_ball_accuracy_percentile ?? 0) : 0} />
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: 32 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Ball carrying</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2px' }}>
                    ...
                    <div style={{ marginBottom: 32 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Physical duels</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2px' }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Defending</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2px' }}>
                  ...
                  <>
                    ...
                    <div style={{ marginBottom: 32 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Goal threat</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2px' }}>
                        <StatRow label={statMode === 'per90' ? 'Goals per 90' : 'Goals'} value={statMode === 'per90' ? fmt(stats.goals_per90) : String(stats.goals ?? 0)} percentile={pct(peerRating?.goals_per90_percentile, peerRating?.goals_raw_percentile, statMode, peerQualified)} />
                        <StatRow label={statMode === 'per90' ? 'Shots per 90' : 'Shots'} value={statMode === 'per90' ? fmt(stats.shots_per90) : String(stats.shots ?? 0)} percentile={pct(peerRating?.shots_per90_percentile, peerRating?.shots_raw_percentile, statMode, peerQualified)} />
            ...
            <Card className="mt-4">
              <CardContent className="p-6">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Peer comparison</span>
                  <div className="flex rounded-lg border border-border bg-card p-0.5 text-xs font-medium">
                    <button
              ...
              {activePeerRating == null ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {activePeerRating.model_score != null && (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: 'var(--muted)', borderRadius: 8 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <span style={{ fontSize: 11, color: 'var(--muted-foreground)', maxWidth: 320 }}>
                          Absolute score based on our model — not peer-relative. 50 = median performer, 100 = theoretical maximum.
                        </span>
                      </div>
                      <span style={{ fontSize: 28, fontWeight: 700, color: Number(activePeerRating.model_score) >= 60 ? '#1d9e75' : Number(activePeerRating.model_score) >= 45 ? '#ef9f27' : '#e24b4a', flexShrink: 0, marginLeft: 16 }}>
                  ...
                  {([
                    ...
                    { label: 'Shot Generation', value: activePeerRating.shot_generation_percentile },
                    { label: 'Chance Creation', value: activePeerRating.chance_creation_percentile },
                    { label: 'Team Function', value: activePeerRating.team_function_percentile },
                    { label: 'Carrying', value: activePeerRating.carrying_percentile },
                    { label: 'Duels', value: activePeerRating.duels_percentile },
                    { label: 'Defensive', value: activePeerRating.defensive_percentile },
