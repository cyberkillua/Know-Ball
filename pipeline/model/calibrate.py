...
Usage:
  python -m pipeline.model.calibrate                    # ST (default), print only
  python -m pipeline.model.calibrate --position W       # Winger, print only
  python -m pipeline.model.calibrate --position W --write  # Winger, update W.json
"""
...
from pipeline.model.engine.normalizer import calibrate_from_raw_scores
...
def main():
    ...
    if not rows:
        log.error(f"No {position} ratings found — run pipeline.model.rate first")
        sys.exit(1)
