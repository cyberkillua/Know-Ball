...
Examples:
    python -m pipeline.historical_backfill --years 10 --dry-run
    python -m pipeline.historical_backfill --years 10
    python -m pipeline.historical_backfill --league 47 --years 10
    python -m pipeline.historical_backfill --status
"""
...
from pipeline.leagues import CURRENT_SEASON, LEAGUES
...
def run_downstream(
    *,
    ...
    if not skip_self_created:
        run_module("pipeline.backfill_self_created", *scoped)
    if full_backfills:
        run_module(
    ...
    run_module("pipeline.rate", "--season", season, "--fotmob-league-id", str(fotmob_id))
    run_module("pipeline.compute", "--season", season, "--fotmob-league-id", str(fotmob_id))
    if not skip_health_check:
        run_module("pipeline.health_check", "--season", season, "--no-strict")
