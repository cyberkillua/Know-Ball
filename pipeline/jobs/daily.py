...
def main() -> None:
    ...
    if args.skip_sofascore_ingest:
        ...
        if args.full_backfills:
            _run("pipeline.ingest.backfill_match_player_extras", *scoped)
        _run("pipeline.ingest.backfill_self_created", *scoped)
        if args.full_backfills:
            _run(
    ...
    _run("pipeline.model.rate")
    _run("pipeline.model.compute")
